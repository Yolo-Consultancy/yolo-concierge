...
export function ContactModal({
  ...
  return (
    ...
    >
      <div
        className="bg-[var(--yolo-cream)] border border-black/10 rounded-2xl w-full max-w-xl my-8 shadow-2xl"
        data-yolo-form
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b yolo-form-divider">
          <h3 className="font-display text-2xl font-bold">Contacter le concierge</h3>
          <button onClick={onClose} className="text-charbon/50 hover:text-charbon transition" aria-label="Fermer">
        ...
        <div className="p-6">
          {submitted ? (
            <div className="text-center py-10">
              <div className="w-16 h-16 mx-auto bg-or-vif flex items-center justify-center mb-6">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="text-charbon">
                  <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" />
                  <path d="M22 4L12 14.01l-3-3" />
                </svg>
              </div>
              <h4 className="font-display text-2xl font-bold mb-3">Demande envoyée</h4>
              <p className="text-sm yolo-form-muted max-w-md mx-auto leading-relaxed">
              ...
              <button
                onClick={onClose}
                className="mt-8 px-6 py-3 bg-charbon hover:bg-or-vif hover:text-charbon text-white text-sm font-medium transition"
              >
                Fermer la fenêtre
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {account && (
                <p className="text-xs text-or-bronze bg-or-vif/10 border border-or-vif/25 px-3 py-2">
                  Vos coordonnées ont été préremplies depuis votre compte.
                </p>
              )}
              {prefilledVehicle && (
                <div className="flex items-center gap-3 p-3 border border-or-vif/30 bg-or-vif/10 text-sm text-or-bronze">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="yolo-form-label">Sujet de la demande</label>
                  <select
                    value={form.subject}
              ...
              <div className="pt-4 border-t yolo-form-divider flex gap-3">
                <button
                  type="button"
                  onClick={onClose}
                  className="flex-1 py-3.5 rounded-xl border border-black/15 text-sm font-medium hover:bg-white text-charbon transition"
                >
                  Annuler
                </button>
                <button
                  type="submit"
                  className="flex-1 py-3.5 rounded-xl bg-or-vif text-charbon text-sm font-semibold hover:bg-charbon hover:text-white transition flex items-center justify-center gap-2"
                >
