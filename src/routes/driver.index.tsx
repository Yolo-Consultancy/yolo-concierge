  ...
  return (
    <div className="max-w-4xl mx-auto w-full">
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        <div className="yolo-card rounded-2xl p-4 border-l-4 border-l-blue-500">
          <p className="text-xs uppercase tracking-wider yolo-muted">En cours</p>
          <p className="text-3xl font-bold text-[var(--yolo-ink)] mt-1">{activeMissions.length}</p>
        </div>
        <div className="yolo-card rounded-2xl p-4 border-l-4 border-l-emerald-500">
          <p className="text-xs uppercase tracking-wider yolo-muted">Terminées</p>
          <p className="text-3xl font-bold text-[var(--yolo-ink)] mt-1">{historyMissions.length}</p>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex gap-2 mb-6 p-1 rounded-xl bg-white border border-black/8">
        <button
          type="button"
          onClick={() => setTab("en_cours")}
          className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all ${
            tab === "en_cours"
              ? "bg-or-vif text-charbon shadow-sm"
              : "yolo-muted hover:text-charbon"
          }`}
        >
      ...
      {loading ? (
        <p className="yolo-muted text-sm">Chargement de vos missions...</p>
      ) : displayed.length === 0 ? (
        <div className="yolo-card rounded-2xl p-10 text-center">
          <p className="yolo-muted text-sm">
            {tab === "en_cours"
      ...
      {reporting && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-charbon border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
...
function MissionCard({
  ...
  return (
    <div className="yolo-card rounded-2xl p-5">
      <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
        <div className="min-w-0 flex-1">
          <p className="font-semibold text-[var(--yolo-ink)] text-lg">{mission.clientName || "Client"}</p>
          <p className="text-sm text-or-bronze mt-0.5">
            {typeLabels[mission.type] || mission.type}
          </p>
        </div>
        <span
          className={`text-xs font-medium px-2.5 py-1 rounded-full border shrink-0 ${
            statusColors[mission.status] || "bg-black/5 text-charbon/60"
          }`}
        >
      ...
      <div className="space-y-1.5 text-sm yolo-muted">
        <p className="flex items-center gap-2">
          <Car className="h-3.5 w-3.5 text-charbon/30 shrink-0" />
          {mission.vehicleName || "Véhicule non renseigné"}
        </p>
        {mission.pickupLocation && (
          <p className="flex items-start gap-2">
            <MapPin className="h-3.5 w-3.5 text-charbon/30 shrink-0 mt-0.5" />
            <span>
              {mission.pickupLocation}
        ...
        {dateRange && (
          <p className="flex items-center gap-2">
            <Calendar className="h-3.5 w-3.5 text-charbon/30 shrink-0" />
            {dateRange}
          </p>
        )}
        <p className="text-xs text-charbon/45">
      ...
      {canReport && onReport && (
        <button
          onClick={onReport}
          className="mt-4 w-full sm:w-auto px-5 py-2.5 rounded-xl bg-or-vif text-charbon text-sm font-semibold hover:bg-charbon hover:text-white transition flex items-center justify-center gap-2"
        >
